...
import PersonCheckBox from '../../../widget/PersonCheckBox'
import Toast from 'react-native-root-toast'
import {GetProblemUserListRequest} from '../../../../api/maillist/MailListRequests'
...
class NoticePersonsScene extends Component {
    ...
    componentWillMount() {
        let self = this;
        this._subscription_noticeConfirm = DeviceEventEmitter.addListener('NOTICE_CONFIRM', function (event) {
            self.props.navigation.goBack();
        });
    }

    componentWillUnmount() {
        this._subscription_noticeConfirm.remove();
    ...
    componentDidMount() {
        let self = this;
        checkPersons = this.props.qbPersons.slice(0);
        ...
        new GetProblemUserListRequest(params).start(function (data) {
            self.setState({
    ...
    _renderContent() {
        ...
        return <View>
            {
                this.state.departmentList && this.state.departmentList.map((item, i) => {
    ...
    _jump2SubDepartment(subDepartment) {
        this.props.navigation.navigate('NoticeSubPersons', {
