...
import {GetAuditProjectGanttRequest} from '../../../../api/auditmanage/AuditManageRequests'
import ColumnDashLine from '../../../widget/ColumnDashLine'
import RowDashLine from '../../../widget/RowDashLine'
import {getApprovalProcessId} from '../../../../actions/approval/ApprovalAction'
import store from '../../../../store/configStore'
const StyleSheet = require('../../../../base/StyleSheet');

let currentTime = 0;
class AuditProjectTree extends Component {
    ...
    _onRefresh() {
        ...
        let params = {processApprovalId: this.props.id};
        new GetAuditProjectGanttRequest(params).start(function (data) {
            self.setState({
                projectPlans: data,
                isRefreshing: false
            })
    ...
    _renderParentItem(projectPlans) {
        ...
        } else if (currentTime < projectPlans.end && currentTime > projectPlans.start) {
            return (
                <View style={styles.longViewItem}>
                    <Text adjustsFontSizeToFit={false} allowFontScaling={false}
    ...
    _renderPlan() {
        ...
        if (null !== projectPlans) {
            store.dispatch(getApprovalProcessId(projectPlans.id));
            v.push(
                <TouchableOpacity key={'level_0'} onPress={() => {
                    store.dispatch(getApprovalProcessId(projectPlans.id));
                    DeviceEventEmitter.emit('GO_DETAIL_PAGE');
                }}>
            ...
            projectPlans.children.map((child, i) => {
                v.push(
                    <View key={'level_1' + i}>
                        <TouchableOpacity
                            onPress={() => {
                                store.dispatch(getApprovalProcessId(child.id));
                                DeviceEventEmitter.emit('GO_DETAIL_PAGE');
                            }}>
                            <View style={styles.rowContainer}>
                                <RowDashLine
            ...
            v.push(
                <View key={'level_2' + i}>
                    <TouchableOpacity
                        onPress={() => {
                            if (ch.children) {
                                ch.children && this.props.navigation.navigate('AuditSubScene', {projectTree: ch})
                            } else {
                                store.dispatch(getApprovalProcessId(ch.id));
                                DeviceEventEmitter.emit('GO_DETAIL_PAGE');
                            }
                        }}>
                        <View style={styles.rowContainer}>
        ...
        return (
            <ScrollView
                ...
                }>
                {
                    ...
                    </View> : <View style={styles.holderContainer}/>
                }
            </ScrollView>
...
export default AuditProjectTree
