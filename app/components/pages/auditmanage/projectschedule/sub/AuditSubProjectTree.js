...
import {getApprovalSubProcessId} from '../../../../../actions/approval/ApprovalAction'
import store from '../../../../../store/configStore'
let currentTime;
class AuditSubProjectTree extends Component {
    ...
    _renderPlan(projectTreeDto) {
        let v = [];
        store.dispatch(getApprovalSubProcessId(projectTreeDto.id));
        v.push(
            <TouchableOpacity
                onPress={() => {
                    store.dispatch(getApprovalSubProcessId(projectTreeDto.id));
            ...
            children && children.length && children.map((child, i) => {
                v.push(
                    <View key={'level_0' + i} style={[styles.rowContainer, {marginTop: 0}]}>
                        <TouchableOpacity
                            onPress={() => {
                                store.dispatch(getApprovalSubProcessId(child.id));
                                DeviceEventEmitter.emit('GO_SUB_DETAIL_PAGE');
                            }}>
                            <View style={styles.rowContainer}>
                                <RowDashLine
                    ...
                    v.push(
                        <View key={'level_1' + j}>
                            <TouchableOpacity
                                onPress={() => {
                                    if (ch.children) {
                                        this.props.navigation.navigate('AuditSubScene', {projectTree: ch})
                                    } else {
                                        store.dispatch(getApprovalSubProcessId(child.id));
                                        DeviceEventEmitter.emit('GO_SUB_DETAIL_PAGE');
                                    }
    ...
    _renderParentItem(item) {
        if (!isNull(item.earlyWarningState) && item.earlyWarningState == 2) {
            ...
            return (
                <View style={styles.longViewItem}>
                    <Text adjustsFontSizeToFit={false} allowFontScaling={false}
                          style={[styles.longViewText, item.canWrite ? {color: '#333333'} : {color: '#999999'}]}>{item.name}
                    </Text>
                    <Text adjustsFontSizeToFit={false} allowFontScaling={false}
        ...
        if (!isNull(child.earlyWarningState) && child.earlyWarningState == 2) {
            ...
            return (
                <View style={styles.subshortViewItem}>
                    <Text adjustsFontSizeToFit={false} allowFontScaling={false}
                          style={[styles.subshortViewText, {color: '#29a1f7'}]}>{child.name}
                    </Text>
                    <Text adjustsFontSizeToFit={false} allowFontScaling={false}
                          style={[styles.subshortViewSubText, {color: '#29a1f7'}]}>{child.dutyNickname}负责</Text>
                </View>
            );
        } else {
            return (
                <View style={styles.subshortViewItem}>
                    <Text adjustsFontSizeToFit={false} allowFontScaling={false}
                          style={[styles.subshortViewText, child.canWrite ? {color: '#333333'} : {color: '#999999'}]}>{child.name}
                    </Text>
                    <Text adjustsFontSizeToFit={false} allowFontScaling={false}
                          style={styles.subshortViewSubText}>{child.dutyNickname}负责</Text>
                </View>
            );
        }
    }
...
export default AuditSubProjectTree